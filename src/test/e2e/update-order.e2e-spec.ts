...
describe('OrderController e2e - Update', () => {
  ...
  it('/order/:id (PUT) - should create a new order', async () => {
    ...
    await request(server).put(`/order/${orderId}`).send(payload).expect(200);
  });
  ...
  it('/order (POST) - Should Fail to Create a new Order', async () => {
    ...
    await request(server).put(`/order/${orderId}`).send(payload).expect(400);
  });
});
