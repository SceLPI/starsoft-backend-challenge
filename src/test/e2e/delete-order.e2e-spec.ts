...
describe('OrderController e2e - Delete', () => {
  ...
  it('/order/:id (DELETE) - Should Delete an Order', async () => {
    const orderId = 'fa23d1b8-41ea-4adf-830e-36743321cb94';

    const server = app.getHttpServer();

    await request(server).delete(`/order/${orderId}`).send().expect(204);
  });

  it('/order/:id (DELETE) - Should Fail to Delete an Order', async () => {
